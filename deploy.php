    ...
    if (!function_exists('shell_exec')) {
        ...
        switch ($action) {
            ...
            case 'push':
                ...
                // Construct the commit command with optional description
                $full_msg = $msg . ($desc ? "\n\n" . $desc : "");
                ...
                break;
            case 'revert':
                // Completely reset the working directory to match the last commit
        ...
        <?php else: ?>
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-b border-white/5 pb-4">
                    <form method="POST">
                        ...
                        <button type="submit" class="w-full h-full flex flex-col items-center justify-center bg-green-600/10 hover:bg-green-600/20 border border-green-600/30 p-4 rounded-2xl transition-all group">
                            ...
                            <span class="block font-bold text-green-400 text-sm">Pull Updates</span>
                            <span class="text-[9px] text-green-500/60 uppercase">Fetch & Merge</span>
                        </button>
                    </form>

                    <form method="POST" onsubmit="return confirm('CRITICAL: This will PERMANENTLY delete all uncommitted local changes to match GitHub. Continue?')">
                        <input type="hidden" name="action" value="revert">
                        <button type="submit" class="w-full h-full flex flex-col items-center justify-center bg-red-600/10 hover:bg-red-600/20 border border-red-600/30 p-4 rounded-2xl transition-all group">
                            <i class="fas fa-undo-alt text-xl text-red-500 mb-2 group-hover:rotate-[-45deg] transition-transform"></i>
                            <span class="block font-bold text-red-400 text-sm">Undo Local</span>
                            <span class="text-[9px] text-red-500/60 uppercase text-center leading-tight">Revert to last commit</span>
                        </button>
                    </form>
                </div>

                <!-- Push Section (Extended) -->
                <form method="POST" class="space-y-3">
                    <input type="hidden" name="action" value="push">
                    
                    <div class="space-y-2">
                        <label class="text-[10px] uppercase tracking-tighter text-slate-500 font-bold ml-1">Commit Highlight / Title</label>
                        <input type="text" name="commit_msg" placeholder="Main Highlight (e.g. Added contact fields)" 
                            class="w-full bg-slate-900/50 border border-slate-700 rounded-xl px-4 py-2 text-sm focus:border-blue-500 outline-none">
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] uppercase tracking-tighter text-slate-500 font-bold ml-1">Extended Description</label>
                        <textarea name="commit_desc" placeholder="Details about changes made..." rows="2"
                            class="w-full bg-slate-900/50 border border-slate-700 rounded-xl px-4 py-2 text-sm focus:border-blue-500 outline-none"></textarea>
                    </div>
                ...
                <div class="flex gap-3">
                    <a href="index.html" class="flex-1 bg-slate-800 hover:bg-slate-700 p-3 rounded-xl text-xs text-center font-bold text-slate-300">Back Home</a>
                    <a href="?logout=1" class="flex-1 bg-slate-800 hover:bg-red-900/40 p-3 rounded-xl text-xs text-center text-red-400 font-bold">Sign Out</a>
                </div>
            </div>
            ...
            <?php if ($output): ?>
                <div class="mt-8">
                    ...
                    <pre class="terminal-box p-5 rounded-2xl text-[10px] md:text-[11px] font-mono leading-relaxed overflow-x-auto whitespace-pre-wrap"><?php 
                        ...
                        // Simple highlighting for common git phrases
